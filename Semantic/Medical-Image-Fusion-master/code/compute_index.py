...
import os
IMAGE_PATH1 = '../test_img/ct/'
IMAGE_PATH2 = '../test_img/mr/'
IMAGE_PATH3 = './test_result_noKL/'
FILE_NAME = './test_result_index_noKL.txt'
...
def comp_idx():
    ...
    EN = 0
    for i in range(18):
        image1 = Image.open(IMAGE_PATH1 + 'ct_' + str(i) + '.png')
        image2 = Image.open(IMAGE_PATH2 + 'mr_' + str(i) + '.png')
        image3 = Image.open(IMAGE_PATH3 + 'fusion_result_' + str(i) + '.png')
        ...
        print(
            ...
            'fusion_img2\t\t{5}\t{6}\t{7}\t{10}\t{13}'.format(psnr1, ssim1, nmi1, mean, stddv, psnr2, ssim2, nmi2,
                                                              entropy, mi1, mi2, sf, cc1, cc2), file=doc)
        print(
            '\t\t\t\tPSNR\t\t\t\tSSIM\t\t\t\tNMI\t\t\t\t\t\tMI\t\t\t\t\t\tCC\t\t\t\tMEAN\t\t\t\tSTDDV\t\t\t\tentropy\t\t\t\tsf\nfusion_img1\t\t{0}\t{1}\t{2}\t{9}\t{12}\t{3}\t{4}\t{8}\t{11}\n'
            'fusion_img2\t\t{5}\t{6}\t{7}\t{10}\t{13}'.format(psnr1, ssim1, nmi1, mean, stddv, psnr2, ssim2, nmi2,
                                                              entropy, mi1, mi2, sf, cc1, cc2))
    SSIM1 /= 18
    SSIM2 /= 18
    NMI1 /= 18
    NMI2 /= 18
    ...
    print('SSIM1: {0}; SSIM2: {1}; NMI1: {2}; NMI2: {3}; CC1: {4}; CC2: {5}; EN: {3}; SF: {4}'.format(SSIM1, SSIM2, NMI1, NMI2, CC1, CC2, EN, SF))
